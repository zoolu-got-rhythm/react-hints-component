...
export function ActorHints({
  actorName,
  top,
  left,
  hintsObj,
  actorImageUrl,
  hintReadingTimeIndicatorColour = "#73fc03",
  onAllHintsRead,
  onExitClicked,
  hintUserReadingTimeInMs = 1500,
  prideColoursOn = false,
  pictureFrameSize = 74,
  imageScalePercentage = "110%",
  ignoreInitialUserInteractionTimeInMs = 2500,
  autoMode = false,
  hintReadingTimeIndicatorBackgroundColour = "#ccc",
  languageKey,
  flipImage = false,
}: ActorHintsProps) {
  ...
  function drawHintTimeoutSemiCircle(progressPercentage: number) {
    ...
    if (ctx) {
      ...
      ctx.lineTo(
        canvasCenter + Math.cos(prevGoingToRadiansAngle) * pictureFrameSize,
        canvasCenter + Math.sin(prevGoingToRadiansAngle) * pictureFrameSize
      );

      let goingToRadiansAngle =
        Math.PI * 1.5 + Math.PI * 2 * progressPercentage;

      ctx.arc(
        canvasCenter,
        canvasCenter,
        pictureFrameSize,
        prevGoingToRadiansAngle,
        goingToRadiansAngle + 0.03
      );
      ...
      if (prideColoursOn) {
        ctx.fillStyle =
          prideColoursRainbow[
            Math.floor(progressPercentage * prideColoursRainbow.length)
          ];
      } else {
  ...
  function delayBeforeNextHint(finishedHint: string) {
    function step(timestamp: number) {
      if (stopAnimBoolRef.current) return;
      ...
      if (elapsed < hintReadingTimeInMs) {
        // Stop the animation after n milliseconds
        ...
        if (hintsFinishedScrollObj[languageKey].length < hints.length) {
          ...
          Object.keys(hintsObj).forEach((langKey) => {
            ...
            hintsFinishedScrollObjCopy[langKey].push(
              hintsObj[langKey][indexOfFinishedHint]
            );
          });
  ...
  useEffect(() => {
    ...
    functionThatExecutesCallbackAfterSpecifiedTimeRef.current =
      getFunctionThatOnlyExecutesAfterTimeElapsed(
        ignoreInitialUserInteractionTimeInMs
      );
    return () => {
  ...
  return (
    ...
    >
      ...
      >
        {`${Math.min(
          hintsFinishedScrollObj[languageKey].length + 1,
          hints.length
        )}/${hints.length}`}
      </div>
      ...
      <HintsBox
        startingHintIndex={
          hintsFinishedScrollObj[languageKey].length === hints.length
            ? hints.length - 1
            : hintsFinishedScrollObj[languageKey].length
        }
        ...
        onSpeechBubbleLayoutChanged={(
          speechBubbleContainerRef: HTMLElement
        ) => {
          hintsCountProgressRef.current.style.left = `${
            pictureFrameSize - 15 + speechBubbleContainerRef.clientWidth
          }px`;
